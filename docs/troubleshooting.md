...
- **Dubious ownership** (shared storage owned by another uid): the failure window offers **Copy the safe.directory fix…** — it puts `git config --global --add safe.directory "<path>"` on the clipboard and opens Termux; paste, Enter, done. The command tells git to trust exactly this directory, which is the same trust the pairing established. It stays a terminal command deliberately: a repository git refuses is one the runner refuses too, so no runner action can reach it.
...
`.git/index.lock` guards the repository while one git process writes. A process the system kills mid-write leaves it behind, and every later operation then fails with `Unable to create '….git/index.lock': File exists`.

Every failure window carrying that message offers **Delete the stale lock…**, and since runner v16 the button looks before it leaps. When a git process is alive and the lock is fresh, that is a command still working, not a leftover: the plugin says so and recommends waiting, because interrupting a write is how repositories end up with empty object files. When nothing that could hold the lock is running, the file is simply deleted — nothing is stopped, an open terminal stays open. Only in between — a lock with unrelated processes alive — does it stop every Termux process first (the window names what stops, a terminal session included) and then delete the file, so nothing can be holding the lock when it goes.
...
The failure window offers **Repair the repository**, and the same repair is a palette command (*Repair the repository (walk every problem)*). It walks every known problem in order — a leftover lock, the git identity and credential scopes, the sparse definition, then the object database — fixing what is safe to fix by itself, narrating into the output panel, and listing anything that needs you (with its button, or the exact Termux command) in the final window. It runs as short steps — Android stopping Termux mid-run loses one step, not the whole repair, and a repair interrupted by closing Obsidian offers to continue on the next launch. For the object database it first removes only object files that are *empty*, which by definition contain nothing, then it asks the remote for exactly the missing objects, by name. On a measured case that was 52 KB against a 3.7 MB history; the output panel lists them as it goes (`asking origin for object 1 of 2`). Objects that are damaged but not empty are reported instead of removed: those may still be recoverable, and that is a decision to make deliberately in Termux. When the walk ends with the repository whole and this device keeps a lightweight copy, it also checks the footprint: a repair that had to download the whole history again brings back everything the lightweight setting had shed, so the final window names how much of that the filter allows shedding again and offers the cleanup as one button. A repository without the lightweight setting is never measured against anything — the full history is what it is configured to hold. Every ending that leaves work behind carries a **Run the repair again** button, so fixing what the window named and walking again never requires the palette.
