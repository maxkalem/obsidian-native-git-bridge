...
- **Re-scan.** The queues are re-scanned after each drain until they are empty, bounded at ten passes. A single snapshot taken at start left every request queued during a long run waiting for the next trigger: the request's own trigger had bounced off the single-instance lock, so a pull queued behind a seven-minute refetch simply sat there until the plugin gave up at its timeout. Whatever a busy device still holds after the tenth pass waits for the next trigger, exactly as everything did before.
...
| 16 | 0.6.6 | Identity and credential scope, value-free: `userNameScopes`/`userEmailScopes`/`credHelperScopes` in status (`--show-scope --name-only`; presence and scope only, never a value), `identity-drop-global`, `cred-helper-local-reset`; the sparse write guard (verified pattern writes with rollback), `sparse-exclude-add` refusing a path with staged content, `repair-sparse-definition`; `repair-triage` (read-only lock/process/leftover facts for the unified repair) and `repair-stale-lock` with `args.skipKill` + `killedProcesses`; exclude changes carry fresh status fields; `-c core.pager=cat` on every git call (a broken `core.pager` only ever bit the interactive runs, which have the tty that engages it); the drain loop re-scans the queues until they are empty (bounded — see the re-scan bullet above); `recoveredCount` on `repair-fetch-missing`, with `recoveredBy` claimed only when it is above zero; `blobDiskKb` in `maintenance-scan` |
| 17 | unreleased | `repair-sparse-definition` refuses a definition holding patterns the plugin's model does not write (an include line other than `/*`, or the exclude-everything `!` + `/*`) instead of rebuilding it — the v16 rebuild kept only `!` lines and destroyed a hand-made whitelist on a real device; when it does rebuild, the previous definition is first copied to `sparse-checkout.ngb-prev` beside the file, and the result carries `sparseBackup`. The verified sparse write also refuses a result containing `!` + `/*`. Interactive-terminal output: the version probe line is framed in dashes, each drained request gets a separator line, and a finished request's verdict token is colored (green ok=true, red ok=false) when stderr is a terminal; requests, results and non-interactive stdout are unchanged from v16 |
...
- `maintenance-scan` is read-only and cheap: raw `git count-objects -v` (whose `garbage`/`size-garbage` fields are exactly where interrupted-download residue lands) plus a size-and-name listing of the pack directory, so the confirmation the plugin shows is built from real numbers. Parsing happens in TypeScript. It also reports `blobDiskKb`, the disk size of every blob the store holds: on a `blob:none` repository that is what a filtered repack can shed, which is how the repair's final window checks a lightweight repository against its own allowance after a refetch has stuffed the packs — a repository without the lightweight marking is never measured against anything, because full history is its configured state.
