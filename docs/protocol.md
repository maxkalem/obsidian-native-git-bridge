...
| 17 | unreleased | Interactive-terminal output only, no protocol change: the version probe line is framed in dashes, each drained request gets a separator line, and a finished request's verdict is colored (green ok=true, red ok=false) when stderr is a terminal. Requests, results and non-interactive stdout are byte-identical to v16 |
